...
| Долговременный | `context.memoryLayers.longTerm` в том же JSON | До явного изменения или удаления агента | Тот же единый обычный текст контекста | `/clear-long` очищает только этот слой |
