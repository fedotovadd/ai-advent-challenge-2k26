...
сохраняются отдельно и не смешиваются с историей диалога. В текстовый prompt
попадают только значения памяти — её внутренние ключи не передаются модели. Долговременная
