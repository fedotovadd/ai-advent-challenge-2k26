...
<body>
  ...
  <script>
    ...
    elements.settingsForm.addEventListener("submit",async(event)=>{event.preventDefault(); if(!state.activeId)return; const maxTokens=elements.maxTokensInput.value.trim(); const settings={systemPrompt:elements.systemPromptInput.value,format:elements.responseFormatInput.value,maxTokens:maxTokens?Number(maxTokens):null,stop:elements.stopInput.value}; elements.saveSettings.disabled=true; elements.settingsStatus.textContent="сохраняем"; try { const {response,body}=await api("/api/sessions/"+state.activeId+"/settings",{method:"PUT",headers:{"Content-Type":"application/json"},body:JSON.stringify(settings)}); if(response.ok&&body.session){upsertSession(body.session); render(); elements.settingsStatus.textContent="сохранено";}else {elements.settingsStatus.textContent="ошибка";setStatus(body.error||"Не удалось сохранить настройки.","error");} } catch(error) { elements.settingsStatus.textContent="ошибка";setStatus("Не удалось соединиться с локальным сервером.","error"); } finally { elements.saveSettings.disabled=false; }});
