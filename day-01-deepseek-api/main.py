...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box; } body { margin:0; min-height:100vh; background:#f8f2ed; } button, textarea, input, select { font:inherit; } button { cursor:pointer; }
    .app { min-height:100vh; display:grid; grid-template-columns:240px minmax(0,1fr) 330px; background:#fffdfa; }
...
<body>
  ...
  <script>
    ...
    function renderFormatInstruction() { elements.formatInstructionField.hidden=elements.responseFormatInput.value==="json"; }
    function renderSettings() { const settings=activeSession()?activeSession().settings:null; if(!settings)return; elements.systemPromptInput.value=settings.systemPrompt; elements.responseFormatInput.value=settings.format; elements.formatInstructionInput.value=settings.formatInstruction; elements.maxTokensInput.value=settings.maxTokens===null?"":settings.maxTokens; elements.stopInput.value=settings.stop; renderFormatInstruction(); }
    ...
    function settingsValues() { const maxTokens=elements.maxTokensInput.value.trim(); return {systemPrompt:elements.systemPromptInput.value,format:elements.responseFormatInput.value,formatInstruction:elements.formatInstructionInput.value,maxTokens:maxTokens?Number(maxTokens):null,stop:elements.stopInput.value}; }
    ...
    function scheduleSettingsSave(delay=500) { const session=activeSession(); if(!session)return; if(state.settingsTimer)clearTimeout(state.settingsTimer); state.pendingSettings={sessionId:session.id,settings:settingsValues()}; elements.settingsStatus.textContent="изменено"; state.settingsTimer=setTimeout(()=>{state.settingsTimer=null;savePendingSettings();},delay); }
    ...
    elements.responseFormatInput.addEventListener("change",()=>{renderFormatInstruction();scheduleSettingsSave(0);});
    [elements.systemPromptInput,elements.formatInstructionInput,elements.maxTokensInput,elements.stopInput].forEach((input)=>input.addEventListener("input",()=>scheduleSettingsSave()));
